...
from model.se_resnet import se_resnet26 as resnet_26
from model.se_resnet import se_resnet50 as resnet50
from model.se_resnet import se_resnet101 as resnet101
from model.decoders import ASPPv3Plus, UbernetDecoder
from model.discriminators import FullyConvDiscriminator
from model.reverse_grad import ReverseLayerF
from model.squeeze import SELayerMultiTaskDict
from model.custom_container import SequentialMultiTask
...
class ResNet(nn.Module):
    def __init__(self, block, layers, n_classes, classifier='atrous-v3', output_stride=16, tasks=None,
        ...
        # Network structure
        self.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=strides[0], padding=3, bias=False)
    ...
    def forward(self, x_in, task=None):
        ...
        # Decoder
        out, features,spk_embedding = self.decoder(x_low, x, task=task)

        # out = F.interpolate(x, size=(in_shape[-2], in_shape[-1]), mode='bilinear', align_corners=False)

        return out, features,spk_embedding

    def compute_losses(self, outputs, features, criteria, gt_elems, alpha, multi_tasks_loss):
        """
        ...
        with torch.enable_grad():
            for task in tasks:
                ...
                # curr_loss = multi_tasks_loss[task] * criteria[task](outputs[task], gt_elems[task])
                # losses_tasks[task] = curr_loss
                # print(outputs[task].view(-1))
                # print(gt_elems[task])
                # outputs_1d=outputs[task].view(-1)
                losses_tasks[task]=criterion(outputs[task],gt_elems[task])

                if self.use_dscr:
                    # Create task labels
                    # task_labels[task] = self._create_task_labels(gt_elems, task).to(outputs[task].device)
                    task_labels[task] = gt_elems[task]
...
def se_resnet26(n_classes, pretrained='scratch', **kwargs):
    ...
    if pretrained == 'imagenet':
        ...
        model_full = resnet_26(pretrained=True)
...
def se_resnet50(n_classes, pretrained='scratch', **kwargs):
    ...
    if pretrained == 'imagenet':
        print('Loading pre-trained ImageNet model')
        model_full = resnet50(pretrained=True)
        model.load_pretrained(model_full)
    elif pretrained == 'scratch':
        print('Training from scratch!')
    else:
...
def se_resnet101(n_classes, pretrained='scratch', **kwargs):
    ...
    if pretrained == 'imagenet':
        print('Loading pre-trained ImageNet model')
        model_full = resnet101(pretrained=True)
        model.load_pretrained(model_full)
    elif pretrained == 'scratch':
        print('Training from scratch!')
    else:
...
#     from attention.fblib.dataloaders import custom_transforms as tr
#     from attention.fblib.dataloaders import PASCALContext
#     from torch.utils.data import DataLoader
#     import attention.fblib.util.visualizepy as viz
#
#     tasks = elems[1:]
#     net.cuda()
#
