...
from model.classifiers_multitask import AtrousSpatialPyramidPoolingModule
from model.squeeze import ConvCoupledSE
...
class UbernetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.bn0 = nn.ModuleDict()
        self.relu = nn.ModuleDict()
        ...
        for task in self.tasks:
            ...
            #                                kernel_size=1,
            #                                bias=True)
            self.predict[task] = nn.Sequential(
                                               nn.Conv2d(in_channels=128,
                                               out_channels=128,
                                               kernel_size=1,bias=False),
                                               nn.BatchNorm2d(128),
                                               )
            self.fc0[task] = nn.Linear(128, 128)
            self.bn0[task] = nn.BatchNorm1d(128)
            self.relu[task] = nn.ReLU()
            self.last[task] = nn.Linear(128, n_classes[task])
    ...
    def forward(self, x_low, x_high, task=None):
        ...
        # print(out.size())
        spk_embedding= self.fc0[task](out)
        # print(spk_embedding.size())
        out = F.relu(self.bn0[task](spk_embedding))  # [batch, n_embed]
        # print(out.size())
        out = self.last[task](out)
        # print(out.size())
