...
def read_MFB(filename):
    ...
    assert len(feature) > 40, (
                'length is too sholabelrt. len:%s, ori_len:%s, file:%s' % (len(feature), len(ori_feat), filename))
...
class DvectorDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(self.spk_to_idx)
        # print(label)
        # print(type(label[0]))
        # print(label[0])
        # print(self.spk_to_idx)
        if( label[0] in self.spk_to_idx.keys()):
            # print("in")
            # print(label[0])
            # print(label[1])
            label[0] = self.spk_to_idx[label[0]]
            # print(type(label[0]))
            # label[1] = self.spk_to_idx[label[1]]
            label[0] = torch.Tensor([label[0]]).long()
            label[1] = torch.Tensor([label[0]]).long()
        # feature=self.loader(feat_path)
        # label=self.DB['speaker_id'][index]
        # label = torch.Tensor([int(label)]).long()
        # label = torch.Tensor([index]).long()
        if self.transform:
...
def main():
    train_DB = read_DB_structure(c.TRAIN_FEAT_DIR)
